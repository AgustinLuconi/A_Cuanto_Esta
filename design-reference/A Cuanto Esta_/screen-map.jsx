/* global React */
// Pantalla 5 — Mapa de cobertura

function ScreenMap({ nav }) {
  const { REGIONS, SUPERMARKETS, SM_BY_ID } = window.ACE_DATA;
  const [active, setActive] = React.useState("amba");
  const region = REGIONS.find(r => r.id === active);

  // Coverage of supermarkets per region
  const REGION_SM = {
  ...
  // Region products comparability (% of catalog comparable)
  const REGION_COV = {
  ...
  return (
    <div className="page page-wide" data-screen-label="05 Mapa">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.1fr 380px", gap: 24, alignItems: "start" }}>
        ...
        <div className="card" style={{ padding: 28, position: "relative" }}>
          <ArgMap active={active} setActive={setActive} regions={REGIONS}/>

          {/* Coverage legend */}
          <div style={{ position: "absolute", left: 28, bottom: 28, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: 12, fontSize: 11.5, color: "var(--fg-2)", display: "flex", flexDirection: "column", gap: 6, boxShadow: "var(--shadow-1)" }}>
            <div style={{ fontWeight: 700, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--fg-3)", marginBottom: 4 }}>Cobertura</div>
          ...
          <div style={{ position: "absolute", right: 28, top: 28, display: "flex", gap: 10 }}>
            <StatPill label="Provincias" value="24" tone="neutral"/>
            <StatPill label="Regiones" value="7" tone="neutral"/>
        ...
        <div className="col" style={{ gap: 16 }}>
          ...
          <div className="card" style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
              <div style={{ fontSize: 11, color: "var(--fg-3)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>Región seleccionada</div>
            ...
            <div style={{ color: "var(--fg-3)", fontSize: 13, marginBottom: 16 }}>
              {region.provs.slice(0, 4).join(", ")}{region.provs.length > 4 ? ` +${region.provs.length-4}` : ""}
            </div>
            ...
            <div style={{ marginBottom: 18 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
                ...
                <span className="mono" style={{ fontSize: 18, fontWeight: 700, color: "var(--primary)" }}>{(REGION_COV[region.id]*100).toFixed(0)}%</span>
              </div>
              ...
              <div style={{ fontSize: 11.5, color: "var(--fg-3)", marginTop: 5 }}>
                <strong className="mono">{Math.floor(3295 * REGION_COV[region.id])}</strong> de 3.295 productos relevados acá
              </div>
            </div>

            {/* supermarkets in region */}
            <div>
              <div style={{ fontSize: 11, color: "var(--fg-3)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Supermercados que operan</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {REGION_SM[region.id].map(s => (
                  <div key={s} style={{
                    ...
                    <SMSwatch sm={s}/>
                    <span style={{ fontSize: 12.5, fontWeight: 500 }}>{SM_BY_ID[s].name}</span>
                  </div>
                ))}
              </div>
            </div>

            <button className="btn" style={{ width: "100%", justifyContent: "center", marginTop: 18 }} onClick={() => nav("search", { query: "leche entera" })}>
              Buscar productos en {region.name}
              <Icon.arrowR />
            </button>
          </div>
              ...
              return (
                ...
                >
                  <div style={{
                    ...
                    background: `oklch(${0.95 - REGION_COV[r.id]*0.5} 0.05 250)`,
...
function CoverageLegend({ pct, label }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{
        width: 16, height: 16, borderRadius: 4,
        background: `oklch(${0.96 - pct * 0.55} 0.06 250)`,
        border: "1px solid var(--border)",
      }}/>
...
// ============================================================================
// Stylized Argentina map (geometric, not real geo)
// ============================================================================
function ArgMap({ active, setActive, regions }) {
  // Geometric polygons that suggest Argentina's shape. Coordinates in a 400x680 viewBox.
  const SHAPES = {
    noa: {
      label: "NOA",
      d: "M 150 40 L 245 35 L 260 130 L 175 145 L 155 110 Z",
      labelPos: [200, 90],
    },
    nea: {
      label: "NEA",
      d: "M 245 35 L 320 60 L 340 165 L 270 175 L 260 130 Z",
      labelPos: [295, 110],
    },
    cuyo: {
      label: "Cuyo",
      d: "M 130 145 L 175 145 L 200 250 L 140 265 L 115 220 Z",
      labelPos: [160, 210],
    },
    centro: {
      label: "Centro",
      d: "M 175 145 L 260 130 L 270 175 L 285 250 L 200 250 Z",
      labelPos: [230, 200],
    },
    pampeana: {
      label: "Pampeana",
      d: "M 140 265 L 200 250 L 285 250 L 305 340 L 220 365 L 155 350 L 135 305 Z",
      labelPos: [220, 310],
    },
    amba: {
      label: "AMBA",
      d: "M 270 280 L 305 280 L 305 340 L 280 345 Z",
      labelPos: [290, 313],
    },
    patagonia: {
      label: "Patagonia",
      d: "M 155 350 L 220 365 L 305 340 L 290 460 L 230 580 L 195 640 L 175 580 L 155 470 L 140 410 Z",
      labelPos: [220, 495],
    },
  };
  ...
  return (
    <div style={{ width: "100%", display: "flex", justifyContent: "center" }}>
      <svg viewBox="0 0 440 680" width="100%" style={{ maxWidth: 460, height: "auto" }}>
        {/* compass + bounding ocean stripes */}
        <defs>
          ...
          return (
            <g key={r.id}>
              <path d={s.d}
                ...
                style={{
                  cursor: "pointer",
                  transition: "fill .15s, transform .2s, filter .2s",
              ...
              />
              {/* label */}
              <text x={s.labelPos[0]} y={s.labelPos[1]}
                textAnchor="middle" dominantBaseline="middle"
                pointerEvents="none"
                style={{
                  fontFamily: "var(--font-sans)",
                  fontSize: 13, fontWeight: 700,
                  fill: isActive ? "white" : "oklch(0.30 0.05 250)",
                }}>
                {s.label}
              </text>
              <text x={s.labelPos[0]} y={s.labelPos[1] + 16}
                textAnchor="middle" dominantBaseline="middle"
                pointerEvents="none"
                style={{
                  fontFamily: "var(--font-mono)",
                  fontSize: 10.5, fontWeight: 600,
                  fill: isActive ? "white" : "oklch(0.50 0.03 250)",
