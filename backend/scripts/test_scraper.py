"""
Script de prueba para CotoScraper.
...
Uso:
    ...
    # Guarda en DB
    python3 scripts/test_scraper.py --save
"""
import sys
import os
...
def main():
    ...
    if save_to_db:
        ...
        try:
            ...
            print(f"\n[coto] Total nuevos productos en DB: {total_new}")
        finally:
            session.close()
    else:
        print("[coto] Modo prueba — solo consola (usar --save para guardar en DB)\n")
        scraper.scrape(
            db_session=None,
            search_terms=SEARCH_TERMS,
            max_per_term=MAX_PER_TERM,
