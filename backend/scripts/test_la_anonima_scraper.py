"""
Script de prueba para LaAnonimaScraper.
...
Uso:
    ...
    python3 scripts/test_la_anonima_scraper.py

    # Guarda en DB
    python3 scripts/test_la_anonima_scraper.py --save
"""
import sys
import os
...
def main():
    ...
    if save_to_db:
        ...
        try:
            ...
            print(f"\n[la_anonima] Total nuevos productos en DB: {total_new}")
        finally:
            session.close()
    else:
        print("[la_anonima] Modo prueba — solo consola (usar --save para guardar en DB)\n")
        scraper.scrape(
            db_session=None,
            search_terms=SEARCH_TERMS,
            max_per_term=MAX_PER_TERM,
